...
for (const auto& entry : gl.Bot3ElementSolution) {
  G4cout << "\tElement: " << entry.first << ", Weight: " << entry.second << G4endl;
	...
	if(gl.VerboseAll > 0 && gl.isBot3)
	  G4cout << "### MaterialBot3 created with " << ncomponents << " components"  << G4endl;
