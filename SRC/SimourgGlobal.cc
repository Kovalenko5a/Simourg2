...
void SimourgGlobal::Dump(void)
{
  ...
  G4cout << "### " << TimeStampDetConstr << G4endl;
  ...
  G4cout << "### " << TimeStampDetHit << G4endl;
  G4cout << "### " << TimeStampMess << G4endl;
  #ifdef G4VIS_USE
...
void SimourgGlobal::E_ASCII_Distr(void) // to call only for E(Event)>0 and VerboseAll > 0
{
	for (G4int j = 0; j < LENGTH_HIT_ASCII_ARR; j++)
	{
		HitEDistr[j] = 0.;
	}
	G4double tLength = tMaxHit - tMinHit;
	G4double eMax = 0.;
	G4double tBin = tLength / (LENGTH_HIT_ASCII_ARR - 1);
	if (tBin == 0) tBin = 1.e-15;
	G4cout << "### tLength = tMaxHit - tMinHit = " << (tLength / s) << " s" << G4endl;
	for(G4int i = 0; i < numHits; i++) // to fill HitEDistr array
	{
		G4int k = int((tHit[i] - tMinHit) / tBin);
		G4cout << "####### k:" << k << " i:" << i << " tHit:" << (tHit[i] / s) << "s; " << G4endl;
		if (k >= LENGTH_HIT_ASCII_ARR) // check respecting the array borders
		{
			G4cout << "### Unexpected k = " << k << " above LENGTH_HIT_ASCII_ARR in decay # " << iRun << ";" << G4endl;
			G4cout << "### tHit = " << (tHit[i] / s) << " s, tMinHit = " << (tMinHit / s) << " s, tMaxHit = " << (tMaxHit / s) << " s" << G4endl;
			k = LENGTH_HIT_ASCII_ARR - 1;
		}
		if (k < 0) // check respecting the array borders
		{
			G4cout << "### Unexpected k = " << k << " below 0 in decay # " << iRun << ";" << G4endl;
			G4cout << "### tHit = " << (tHit[i] / s) << " s, tMinHit = " << (tMinHit / s) << " s, tMaxHit = " << (tMaxHit / s) << " s" << G4endl;
			k = 0;
		}
		HitEDistr[k] += eHit[i];
		if (HitEDistr[k] > eMax) eMax = HitEDistr[k];
	}
	G4double eBin = eMax / HEIGHT_HIT_ASCII_ARR;
	for (G4int j = 0; j < LENGTH_HIT_ASCII_ARR; j++)
	{
		G4int kH = int(HitEDistr[j] / eBin);
		for (G4int jj = 0; jj < HEIGHT_HIT_ASCII_ARR; jj++)
		{
			ASCII_Distr[j][jj] = (jj < kH ? '*' : ' ');
		}
	}
	for (G4int jj = HEIGHT_HIT_ASCII_ARR - 1; jj >= 0; jj--)
	{
		G4cout << "### |";
		for (G4int j = 0; j < LENGTH_HIT_ASCII_ARR; j++)
		{
			G4cout << ASCII_Distr[j][jj];
		}
		G4cout << G4endl;
	}
	G4cout   << "### +";
	...
	G4cout << "  " << (tMaxHit / s) << " s" << G4endl;
	G4cout << (tMinHit / s) << " s" << G4endl;
}
